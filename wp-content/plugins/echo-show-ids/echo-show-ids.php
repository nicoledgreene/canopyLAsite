<?php
/**
 * Plugin Name: Show IDs by Echo Plugins
...
final class Echo_Show_IDs {
	...
	/**
	 * Setup class auto-loading and other support functions
	 */
...
function epsi_show_admin_notices() {
	...
	if ( empty($_GET['epsi_admin_notice']) ) {
		return;
	}

	$type = 'error';
	switch ( $_GET['epsi_admin_notice'] ) {

		case 'ep_settings_saved' :
			$message = __( 'Settings saved', 'epsi' );
			$type   = 'success';
			break;
		case 'ep_refresh_page' :
			$message = __( 'Refresh your page', 'epsi' );
			break;
		case 'ep_refresh_page_error' :
			$message = __( 'Error occurred. Please refresh your browser and try again', 'epsi' );
			break;
		case 'ep_security_failed' :
			$message = __( 'You do not have permission', 'epsi' );
			break;
		default:
