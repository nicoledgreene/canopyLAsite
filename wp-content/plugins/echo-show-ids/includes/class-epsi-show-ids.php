...
class EPSI_Show_IDs {
	...
	public function __construct() {
		...
		add_action( 'admin_body_class', array($this, 'add_ID_to_columns_list') );
		add_action( 'admin_body_class', array($this, 'add_ID_to_action_rows') );
	 ...
	 */
	public function add_ID_to_columns_list( $classes ) {
		...
		// Media List
		add_filter( 'manage_media_columns', array( $this, 'add_column_heading' ), 9, 1 );
	...
	public function add_column_value( $column_name, $post_id ) {
		if ( ! empty($column_name) && $column_name == 'epsi_column_id' ) {
			echo $post_id;
		}
	}

	public function add_column_value_new($value, $column_name, $post_id) {
		return $column_name == 'epsi_column_id' ? $post_id : $value;
	}
	 ...
	 */
	public function add_ID_to_action_rows( $classes ) {

		// check if we should display the list of IDs
		if ( ! $this->epsi_is_option_on( 'where_to_display_ids', 'epsi-show-inline' ) ) {
			return $classes;
		}
